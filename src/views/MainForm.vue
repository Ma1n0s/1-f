  ...
  <div class="min-h-screen flex flex-col">
    ...
    <main class="py-4 bg-white text-black flex-1">
      <div>
        ...
        <div class="bg-gray-300 p-6 w-full h-96 mx-auto rounded shadow-md flex items-center justify-center">
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
      <div>
    <comp @companyConfirmed="addCompany" :companies="companies" />
    <ul>
      <li v-for="company in companies" :key="company.id" @click="goToCompany(company.id)">
        {{ company.name }}
      </li>
    </ul>
  </div>
  ...
  methods: {
    async fetchCompanies() {
      try {
        ...
        this.companies = await response.json(); // Сохраняем данные в массив
      } catch (error) {
        console.error('Ошибка при получении компаний:', error);
      }
    },
    goToCompany(companyId) {
      // Перенаправление на страницу с информацией о компании
